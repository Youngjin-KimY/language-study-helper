import { ProgressMsg, DownloadRequest, TrasncribeRequest, MergeRequest} from "./type.js";
...
async function showJsonSubtitles(videoEl: HTMLVideoElement, jsonUrl: string) {
...
export function start() {
    const urlInput = $(Ids.UrlInput) as HTMLInputElement;
    ...
    function startTrascribe(filename: string) {
        const langSelect:HTMLSelectElement = $(Ids.LanguageSelector) as HTMLSelectElement; 
        const lang: string = langSelect.value;
        let newSocket = new WebSocket(webSocketFullPath);
        newSocket.onopen = () => {
            const req: TrasncribeRequest = {
        ...
        newSocket.onmessage = (event) => {
            const data = JSON.parse(event.data) as ProgressMsg;
            switch (data.step) {
                case Step.Ing:
                    $(Ids.Transcribe).textContent = `Transcribe: ${data.progress?.toFixed(1)}`;
                    break;
    ...
    function startMerge(filename: string) {
...
async function fetchVideoList() {
    ...
    container!.innerHTML = list.map((f: string) => {
    ...
    container!.querySelectorAll("a").forEach(link => {
        link.addEventListener("click", async (e) => {
            e.preventDefault();
            const base = (e.currentTarget as HTMLElement).getAttribute("data-base")!;
            ...
            let jsonExists = "";
            try {
                const check = await fetch(subtitleSrc, { method: "HEAD" });
                if (check.ok) {
            ...
            const videoEl = document.querySelector("#videoPlayerContainer video") as HTMLVideoElement;
